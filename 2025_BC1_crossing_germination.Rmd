...
germ_avg <- germ_long %>%
  ...
  summarize(Avg_germ_prop = mean(Germ_prop, na.rm=TRUE),
            ...
            Avg_germ_rate = mean(Germ_rate_seed, na.rm=TRUE),
            SD_germ_rate = sd(Germ_rate_seed, na.rm=TRUE),
            n = sum(!is.na(Germ_rate_seed)),  # Count non-NA values for Germ_rate_seed
    SE_germ_rate = SD_germ_rate / sqrt(n),  
    .groups = "drop")

overall_avg_germ_rate <- germ_avg %>%
  group_by(dame_pop, sire_pop) %>%
  summarise(
    Avg_germ_rate_all = mean(Avg_germ_rate, na.rm=TRUE),
    SD_germ_rate_all = sd(Avg_germ_rate, na.rm=TRUE),
...
```{r}
Germ_rate_plot <- ggplot(overall_avg_germ_rate, aes(x=dame_pop, y=Avg_germ_rate_all)) + 
  geom_point() +
...
```
```{r}
Germ_prop_plot <- ggplot(germ_avg, aes(x=Days_to_germ, y=Avg_germ_prop)) +
  geom_line(size=1)+
